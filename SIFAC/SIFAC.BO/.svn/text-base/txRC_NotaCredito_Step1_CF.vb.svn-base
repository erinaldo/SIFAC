...
Partial Public Class txRC_NotaCredito_Step1		
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 300 Then
					Throw New ArgumentOutOfRangeException("Descripcion", Value.ToString(), "Valor inv?lido para txRC_NotaCredito_Step1.Descripcion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (300).")
				End If
			End If
			m_Descripcion = Value
		End Set
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 50 Then
					Throw New ArgumentOutOfRangeException("UsuarioCreacion", Value.ToString(), "Valor inv?lido para txRC_NotaCredito_Step1.UsuarioCreacion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (50).")
				End If
			End If
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 10 Then
					Throw New ArgumentOutOfRangeException("objSccCuentaID", Value.ToString(), "Valor inv?lido para txRC_NotaCredito_Step1.objSccCuentaID. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (10).")
				End If
			End If
...
#Region " Batch Update " 
	''' <summary>
    ''' Salva un DataSet del tipo de la tabla txRC_NotaCredito_Step1 en la base de datos.
    ''' </summary>
    ''' <param name="DS">Dataset a salvar. El DS debe necesariamente ser un select a la tabla txRC_NotaCredito_Step1 </param>    
	...
	public Shared Sub BatchUpdate( DS as Dataset, optional Byval pTransac as TransactionManager = Nothing )
		...
		Try
			...
			cmdDelete.CommandText = "DELETE FROM  txRC_NotaCredito_Step1 WHERE SccNotaCreditoID= @SccNotaCreditoID" 	
			...
			cmdInsert.CommandText = "INSERT INTO txRC_NotaCredito_Step1 ( Numero, Fecha, Monto, objConceptoID, Descripcion, objEstadoID, UsuarioCreacion, FechaCreacion, objSccCuentaID, objTiendaID) VALUES ( @Numero, @Fecha, @Monto, @objConceptoID, @Descripcion, @objEstadoID, @UsuarioCreacion, @FechaCreacion, @objSccCuentaID, @objTiendaID)"
			...
			cmdUpdate.CommandText = "UPDATE txRC_NotaCredito_Step1 SET Numero=@Numero, Fecha=@Fecha, Monto=@Monto, objConceptoID=@objConceptoID, Descripcion=@Descripcion, objEstadoID=@objEstadoID, UsuarioCreacion=@UsuarioCreacion, FechaCreacion=@FechaCreacion, objSccCuentaID=@objSccCuentaID, objTiendaID=@objTiendaID WHERE SccNotaCreditoID= @wSccNotaCreditoID"
			...
			sqlHelper.UpdateDataset (cmdInsert, cmdDelete, cmdUpdate, DS, "txRC_NotaCredito_Step1")
...
#Region " Retrieve "
	''' <summary>
    ''' Devuelve de la base de datos un objeto txRC_NotaCredito_Step1
    ''' </summary>    
    ...
    Public Function Retrieve(Byval p_SccNotaCreditoID as Integer, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from txRC_NotaCredito_Step1 where " & " SccNotaCreditoID = " & p_SccNotaCreditoID		
	...
	''' <summary>
    ''' Devuelve de la base de datos un objeto txRC_NotaCredito_Step1 usando un filter.
    ''' </summary>
    ''' <param name="pFilter">Filtro a aplicar en el select que carga la clase</param>    
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Retrieve.</param>    
    Public Function RetrieveByFilter(ByVal pFilter as String, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from txRC_NotaCredito_Step1 where " & pFilter				
        Dim dr As SqlDataReader = Nothing
	...
	''' <summary>
    ''' Trae de la base de datos un conjunto de objetos txRC_NotaCredito_Step1 en un objeto DataTable
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns> 
    Public Shared Function RetrieveDT(Optional ByVal sFilter As String = "", Optional ByVal OrderBy As String = "", Optional Byval pFields as String = "*", optional Byval pTransac as TransactionManager = Nothing) As DataTable
        Dim sSQL As String = "select " & pFields &" from txRC_NotaCredito_Step1"
		...
		Try
			...
			End If
			ds.Tables(0).TableName = "txRC_NotaCredito_Step1"
			Return (ds.Tables(0))
		Catch ex As Exception
			Throw        
        End Try		
	...
	''' <summary>
    ''' Trae de la base de datos un conjunto de objetos txRC_NotaCredito_Step1 en un objeto DataSet
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns> 
    ...
    ''' <summary>
    ''' Trae de la base de datos un conjunto de objetos txRC_NotaCredito_Step1 en un objeto DataReader
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns>
...
#Region " Insert "
	...
	''' <summary>
    ''' 	Inserta en la base de datos una nueva instancia de la clase txRC_NotaCredito_Step1.
    ''' </summary>	
    ''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Insert.</param>
    ''' <remarks></remarks>
	Public Sub Insert(Optional ByRef pTransac As TransactionManager = Nothing)
		Dim sCommand As String = "insert into txRC_NotaCredito_Step1("
		sCommand &= "Numero,"
		sCommand &= "Fecha,"
		sCommand &= "Monto,"
		...
		sCommand &= " select "
		sCommand &= "@SccNotaCreditoID = SccNotaCreditoID from txRC_NotaCredito_Step1 where "		
        ...
        End If
		arParams(9) = New SqlParameter("@objSccCuentaID", SqlDbType.VarChar)		
		If IsDBNull(m_objSccCuentaID) Then
            arParams(9).Value = DBNull.Value
        Else
            arParams(9).Value = m_objSccCuentaID
        End If
		arParams(10) = New SqlParameter("@objTiendaID", SqlDbType.Int)		
		If IsDBNull(m_objTiendaID) Then
            arParams(10).Value = DBNull.Value
        Else
            arParams(10).Value = m_objTiendaID
        End If
...
#Region " Update "
	...
	''' <summary>
    ''' 	Actualiza el objeto txRC_NotaCredito_Step1 en la base de datos.
    ''' </summary>	
    ''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    ''' <remarks></remarks>
	Public Sub Update(Optional ByRef pTransac As TransactionManager = Nothing)        		
		Dim sCommand As String = "update txRC_NotaCredito_Step1 set "		
...
#Region " Delete "
	...
	''' <summary>
    ''' Borra un objeto txRC_NotaCredito_Step1 de la base de datos.
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    ''' </summary>    
    Public Sub Delete(optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from txRC_NotaCredito_Step1 where " & " SccNotaCreditoID = " & m_SccNotaCreditoID
			If pTransac Is Nothing Then
    ...
    Public Shared Sub Delete(Byval p_SccNotaCreditoID as Integer, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from txRC_NotaCredito_Step1 where " & " SccNotaCreditoID = " & p_SccNotaCreditoID
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
    ...
    ''' <summary>
    ''' Borra objetos txRC_NotaCredito_Step1 de la base de datos en base al par?metro WHERE
    ''' </summary>
    ''' <param name="pWhere">Condici?n usada para borrar los objetos de la base de datos.</param>
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    Public Shared Sub DeleteByFilter(Byval pWhere as String, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from txRC_NotaCredito_Step1 where " & pWhere
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
