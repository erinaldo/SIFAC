...
Partial Public Class txRC_ReciboDetFactura_Step2		
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 50 Then
					Throw New ArgumentOutOfRangeException("UsuarioCreacion", Value.ToString(), "Valor inv?lido para txRC_ReciboDetFactura_Step2.UsuarioCreacion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (50).")
				End If
			End If
...
#Region " Batch Update " 
	''' <summary>
    ''' Salva un DataSet del tipo de la tabla txRC_ReciboDetFactura_Step2 en la base de datos.
    ''' </summary>
    ''' <param name="DS">Dataset a salvar. El DS debe necesariamente ser un select a la tabla txRC_ReciboDetFactura_Step2 </param>    
	...
	public Shared Sub BatchUpdate( DS as Dataset, optional Byval pTransac as TransactionManager = Nothing )
		...
		Try
			...
			cmdDelete.CommandText = "DELETE FROM  txRC_ReciboDetFactura_Step2 WHERE SccReciboDetFacturaID= @SccReciboDetFacturaID" 	
			...
			cmdInsert.CommandText = "INSERT INTO txRC_ReciboDetFactura_Step2 ( objFacturaID, Monto, EsAbono, objReciboCajaID, UsuarioCreacion, FechaCreacion) VALUES ( @objFacturaID, @Monto, @EsAbono, @objReciboCajaID, @UsuarioCreacion, @FechaCreacion)"
			...
			cmdUpdate.CommandText = "UPDATE txRC_ReciboDetFactura_Step2 SET objFacturaID=@objFacturaID, Monto=@Monto, EsAbono=@EsAbono, objReciboCajaID=@objReciboCajaID, UsuarioCreacion=@UsuarioCreacion, FechaCreacion=@FechaCreacion WHERE SccReciboDetFacturaID= @wSccReciboDetFacturaID"
			...
			sqlHelper.UpdateDataset (cmdInsert, cmdDelete, cmdUpdate, DS, "txRC_ReciboDetFactura_Step2")
...
#Region " Retrieve "
	''' <summary>
    ''' Devuelve de la base de datos un objeto txRC_ReciboDetFactura_Step2
    ''' </summary>    
    ...
    Public Function Retrieve(Byval p_SccReciboDetFacturaID as Integer, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from txRC_ReciboDetFactura_Step2 where " & " SccReciboDetFacturaID = " & p_SccReciboDetFacturaID		
	...
	''' <summary>
    ''' Devuelve de la base de datos un objeto txRC_ReciboDetFactura_Step2 usando un filter.
    ''' </summary>
    ''' <param name="pFilter">Filtro a aplicar en el select que carga la clase</param>    
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Retrieve.</param>    
    Public Function RetrieveByFilter(ByVal pFilter as String, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from txRC_ReciboDetFactura_Step2 where " & pFilter				
        Dim dr As SqlDataReader = Nothing
	...
	''' <summary>
    ''' Trae de la base de datos un conjunto de objetos txRC_ReciboDetFactura_Step2 en un objeto DataTable
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns> 
    Public Shared Function RetrieveDT(Optional ByVal sFilter As String = "", Optional ByVal OrderBy As String = "", Optional Byval pFields as String = "*", optional Byval pTransac as TransactionManager = Nothing) As DataTable
        Dim sSQL As String = "select " & pFields &" from txRC_ReciboDetFactura_Step2"
		...
		Try
			...
			End If
			ds.Tables(0).TableName = "txRC_ReciboDetFactura_Step2"
			Return (ds.Tables(0))
		Catch ex As Exception
			Throw        
        End Try		
	...
	''' <summary>
    ''' Trae de la base de datos un conjunto de objetos txRC_ReciboDetFactura_Step2 en un objeto DataSet
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns> 
    ...
    ''' <summary>
    ''' Trae de la base de datos un conjunto de objetos txRC_ReciboDetFactura_Step2 en un objeto DataReader
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns>
...
#Region " Insert "
	...
	''' <summary>
    ''' 	Inserta en la base de datos una nueva instancia de la clase txRC_ReciboDetFactura_Step2.
    ''' </summary>	
    ''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Insert.</param>
    ''' <remarks></remarks>
	Public Sub Insert(Optional ByRef pTransac As TransactionManager = Nothing)
		Dim sCommand As String = "insert into txRC_ReciboDetFactura_Step2("
		...
		sCommand &= " select "
		sCommand &= "@SccReciboDetFacturaID = SccReciboDetFacturaID from txRC_ReciboDetFactura_Step2 where "		
...
#Region " Update "
	...
	''' <summary>
    ''' 	Actualiza el objeto txRC_ReciboDetFactura_Step2 en la base de datos.
    ''' </summary>	
    ''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    ''' <remarks></remarks>
	Public Sub Update(Optional ByRef pTransac As TransactionManager = Nothing)        		
		Dim sCommand As String = "update txRC_ReciboDetFactura_Step2 set "		
...
#Region " Delete "
	...
	''' <summary>
    ''' Borra un objeto txRC_ReciboDetFactura_Step2 de la base de datos.
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    ''' </summary>    
    Public Sub Delete(optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from txRC_ReciboDetFactura_Step2 where " & " SccReciboDetFacturaID = " & m_SccReciboDetFacturaID
			If pTransac Is Nothing Then
    ...
    Public Shared Sub Delete(Byval p_SccReciboDetFacturaID as Integer, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from txRC_ReciboDetFactura_Step2 where " & " SccReciboDetFacturaID = " & p_SccReciboDetFacturaID
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
    ...
    ''' <summary>
    ''' Borra objetos txRC_ReciboDetFactura_Step2 de la base de datos en base al par?metro WHERE
    ''' </summary>
    ''' <param name="pWhere">Condici?n usada para borrar los objetos de la base de datos.</param>
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    Public Shared Sub DeleteByFilter(Byval pWhere as String, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from txRC_ReciboDetFactura_Step2 where " & pWhere
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
