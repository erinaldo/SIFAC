...
Partial Public Class txRC_FacturaDetalle_Step3		
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 15 Then
					Throw New ArgumentOutOfRangeException("objSccCuentaID", Value.ToString(), "Valor inv?lido para txRC_FacturaDetalle_Step3.objSccCuentaID. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (15).")
				End If
			End If
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 50 Then
					Throw New ArgumentOutOfRangeException("UsuarioCreacion", Value.ToString(), "Valor inv?lido para txRC_FacturaDetalle_Step3.UsuarioCreacion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (50).")
				End If
			End If
...
#Region " Batch Update " 
	''' <summary>
    ''' Salva un DataSet del tipo de la tabla txRC_FacturaDetalle_Step3 en la base de datos.
    ''' </summary>
    ''' <param name="DS">Dataset a salvar. El DS debe necesariamente ser un select a la tabla txRC_FacturaDetalle_Step3 </param>    
	...
	public Shared Sub BatchUpdate( DS as Dataset, optional Byval pTransac as TransactionManager = Nothing )
		...
		Try
			...
			cmdDelete.CommandText = "DELETE FROM  txRC_FacturaDetalle_Step3 WHERE SccCuentaPorCobrarDetalleID= @SccCuentaPorCobrarDetalleID" 	
			...
			cmdInsert.CommandText = "INSERT INTO txRC_FacturaDetalle_Step3 ( objSccCuentaID, objTiendaID, objFacturaID, Saldo, SaldoND, MontoTotal, MontoPrima, MontoAbonado, MontoCuota, objTeminoPlazoID, objEstadoID, Descuento, objCalificacionID, FechaUltimoPago, FechaProximoPago, FechaVencimiento, UsuarioCreacion, FechaCreacion) VALUES ( @objSccCuentaID, @objTiendaID, @objFacturaID, @Saldo, @SaldoND, @MontoTotal, @MontoPrima, @MontoAbonado, @MontoCuota, @objTeminoPlazoID, @objEstadoID, @Descuento, @objCalificacionID, @FechaUltimoPago, @FechaProximoPago, @FechaVencimiento, @UsuarioCreacion, @FechaCreacion)"
			...
			sqlHelper.UpdateDataset (cmdInsert, cmdDelete, cmdUpdate, DS, "txRC_FacturaDetalle_Step3")
...
#Region " Retrieve "
	''' <summary>
    ''' Devuelve de la base de datos un objeto txRC_FacturaDetalle_Step3
    ''' </summary>    
    ...
    Public Function Retrieve(Byval p_SccCuentaPorCobrarDetalleID as Integer, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from txRC_FacturaDetalle_Step3 where " & " SccCuentaPorCobrarDetalleID = " & p_SccCuentaPorCobrarDetalleID		
	...
	''' <summary>
    ''' Devuelve de la base de datos un objeto txRC_FacturaDetalle_Step3 usando un filter.
    ''' </summary>
    ''' <param name="pFilter">Filtro a aplicar en el select que carga la clase</param>    
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Retrieve.</param>    
    Public Function RetrieveByFilter(ByVal pFilter as String, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from txRC_FacturaDetalle_Step3 where " & pFilter				
        Dim dr As SqlDataReader = Nothing
	...
	''' <summary>
    ''' Trae de la base de datos un conjunto de objetos txRC_FacturaDetalle_Step3 en un objeto DataTable
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns> 
    Public Shared Function RetrieveDT(Optional ByVal sFilter As String = "", Optional ByVal OrderBy As String = "", Optional Byval pFields as String = "*", optional Byval pTransac as TransactionManager = Nothing) As DataTable
        Dim sSQL As String = "select " & pFields &" from txRC_FacturaDetalle_Step3"
		...
		Try
			...
			End If
			ds.Tables(0).TableName = "txRC_FacturaDetalle_Step3"
			Return (ds.Tables(0))
		Catch ex As Exception
			Throw        
        End Try		
	...
	''' <summary>
    ''' Trae de la base de datos un conjunto de objetos txRC_FacturaDetalle_Step3 en un objeto DataSet
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns> 
    ...
    ''' <summary>
    ''' Trae de la base de datos un conjunto de objetos txRC_FacturaDetalle_Step3 en un objeto DataReader
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns>
...
#Region " Insert "
	...
	''' <summary>
    ''' 	Inserta en la base de datos una nueva instancia de la clase txRC_FacturaDetalle_Step3.
    ''' </summary>	
    ''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Insert.</param>
    ''' <remarks></remarks>
	Public Sub Insert(Optional ByRef pTransac As TransactionManager = Nothing)
		Dim sCommand As String = "insert into txRC_FacturaDetalle_Step3("
		...
		sCommand &= " select "
		sCommand &= "@SccCuentaPorCobrarDetalleID = SccCuentaPorCobrarDetalleID from txRC_FacturaDetalle_Step3 where "		
        ...
        End If
		arParams(13) = New SqlParameter("@objCalificacionID", SqlDbType.Int)		
		If IsDBNull(m_objCalificacionID) Then
            arParams(13).Value = DBNull.Value
        Else
            arParams(13).Value = m_objCalificacionID
        End If
		arParams(14) = New SqlParameter("@FechaUltimoPago", SqlDbType.DateTime)		
		If IsDBNull(m_FechaUltimoPago) Then
            arParams(14).Value = DBNull.Value
        Else
            arParams(14).Value = m_FechaUltimoPago
        End If
		arParams(15) = New SqlParameter("@FechaProximoPago", SqlDbType.DateTime)		
		If IsDBNull(m_FechaProximoPago) Then
            arParams(15).Value = DBNull.Value
        Else
            arParams(15).Value = m_FechaProximoPago
        End If
		arParams(16) = New SqlParameter("@FechaVencimiento", SqlDbType.DateTime)		
		If IsDBNull(m_FechaVencimiento) Then
            arParams(16).Value = DBNull.Value
        Else
            arParams(16).Value = m_FechaVencimiento
        End If
...
#Region " Update "
	...
	''' <summary>
    ''' 	Actualiza el objeto txRC_FacturaDetalle_Step3 en la base de datos.
    ''' </summary>	
    ''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    ''' <remarks></remarks>
	Public Sub Update(Optional ByRef pTransac As TransactionManager = Nothing)        		
		Dim sCommand As String = "update txRC_FacturaDetalle_Step3 set "		
...
#Region " Delete "
	...
	''' <summary>
    ''' Borra un objeto txRC_FacturaDetalle_Step3 de la base de datos.
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    ''' </summary>    
    Public Sub Delete(optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from txRC_FacturaDetalle_Step3 where " & " SccCuentaPorCobrarDetalleID = " & m_SccCuentaPorCobrarDetalleID
			If pTransac Is Nothing Then
    ...
    Public Shared Sub Delete(Byval p_SccCuentaPorCobrarDetalleID as Integer, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from txRC_FacturaDetalle_Step3 where " & " SccCuentaPorCobrarDetalleID = " & p_SccCuentaPorCobrarDetalleID
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
    ...
    ''' <summary>
    ''' Borra objetos txRC_FacturaDetalle_Step3 de la base de datos en base al par?metro WHERE
    ''' </summary>
    ''' <param name="pWhere">Condici?n usada para borrar los objetos de la base de datos.</param>
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    Public Shared Sub DeleteByFilter(Byval pWhere as String, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from txRC_FacturaDetalle_Step3 where " & pWhere
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
