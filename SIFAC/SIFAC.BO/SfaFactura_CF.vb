...
Partial Public Class SfaFactura		
...
#Region " Propiedades "
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 10 Then
					Throw New ArgumentOutOfRangeException("Numero", Value.ToString(), "Valor inv?lido para SfaFactura.Numero. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (10).")
				End If
			End If
			m_Numero = Value
		End Set
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 20 Then
					Throw New ArgumentOutOfRangeException("Chasis", Value.ToString(), "Valor inv?lido para SfaFactura.Chasis. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (20).")
				End If
			End If
			m_Chasis = Value
		End Set
	...
	Public Property NoMotor() As String
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 20 Then
					Throw New ArgumentOutOfRangeException("NoMotor", Value.ToString(), "Valor inv?lido para SfaFactura.NoMotor. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (20).")
				End If
			End If
			m_NoMotor = Value
		End Set
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 50 Then
					Throw New ArgumentOutOfRangeException("UsuarioCreacion", Value.ToString(), "Valor inv?lido para SfaFactura.UsuarioCreacion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (50).")
				End If
			End If
	...
	Public Property UsuarioModificacion() As String
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 50 Then
					Throw New ArgumentOutOfRangeException("UsuarioModificacion", Value.ToString(), "Valor inv?lido para SfaFactura.UsuarioModificacion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (50).")
				End If
			End If
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 15 Then
					Throw New ArgumentOutOfRangeException("SccCuentaID", Value.ToString(), "Valor inv?lido para SfaFactura.SccCuentaID. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (15).")
				End If
			End If
			m_SccCuentaID = Value
		End Set
...
#Region " Batch Update " 
	''' <summary>
    ''' Salva un DataSet del tipo de la tabla SfaFactura en la base de datos.
    ''' </summary>
    ''' <param name="DS">Dataset a salvar. El DS debe necesariamente ser un select a la tabla SfaFactura </param>    
	...
	public Shared Sub BatchUpdate( DS as Dataset, optional Byval pTransac as TransactionManager = Nothing )
		...
		Try
			...
			cmdDelete.CommandText = "DELETE FROM  SfaFactura WHERE SfaFacturaID= @SfaFacturaID" 	
			...
			cmdInsert.CommandText = "INSERT INTO SfaFactura ( Fecha, objProductoID, objConceptoFactura, Numero, Chasis, NoMotor, MontoTotal, objEmpleadoID, UsuarioCreacion, FechaCreacion, UsuarioModificacion, FechaModificacion, SccCuentaID) VALUES ( @Fecha, @objProductoID, @objConceptoFactura, @Numero, @Chasis, @NoMotor, @MontoTotal, @objEmpleadoID, @UsuarioCreacion, @FechaCreacion, @UsuarioModificacion, @FechaModificacion, @SccCuentaID)"
			...
			cmdUpdate.CommandText = "UPDATE SfaFactura SET Fecha=@Fecha, objProductoID=@objProductoID, objConceptoFactura=@objConceptoFactura, Numero=@Numero, Chasis=@Chasis, NoMotor=@NoMotor, MontoTotal=@MontoTotal, objEmpleadoID=@objEmpleadoID, UsuarioCreacion=@UsuarioCreacion, FechaCreacion=@FechaCreacion, UsuarioModificacion=@UsuarioModificacion, FechaModificacion=@FechaModificacion, SccCuentaID=@SccCuentaID WHERE SfaFacturaID= @wSfaFacturaID"
			...
			sqlHelper.UpdateDataset (cmdInsert, cmdDelete, cmdUpdate, DS, "SfaFactura")
...
#Region " Retrieve "
	''' <summary>
    ''' Devuelve de la base de datos un objeto SfaFactura
    ''' </summary>    
    ...
    Public Function Retrieve(Byval p_SfaFacturaID as Integer, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from SfaFactura where " & " SfaFacturaID = " & p_SfaFacturaID		
	...
	''' <summary>
    ''' Devuelve de la base de datos un objeto SfaFactura usando un filter.
    ''' </summary>
    ''' <param name="pFilter">Filtro a aplicar en el select que carga la clase</param>    
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Retrieve.</param>    
    Public Function RetrieveByFilter(ByVal pFilter as String, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from SfaFactura where " & pFilter				
        Dim dr As SqlDataReader = Nothing
	...
	''' <summary>
    ''' Trae de la base de datos un conjunto de objetos SfaFactura en un objeto DataTable
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns> 
    Public Shared Function RetrieveDT(Optional ByVal sFilter As String = "", Optional ByVal OrderBy As String = "", Optional Byval pFields as String = "*", optional Byval pTransac as TransactionManager = Nothing) As DataTable
        Dim sSQL As String = "select " & pFields &" from SfaFactura"
		...
		Try
			...
			End If
			ds.Tables(0).TableName = "SfaFactura"
			Return (ds.Tables(0))
		Catch ex As Exception
			Throw        
        End Try		
	...
	''' <summary>
    ''' Trae de la base de datos un conjunto de objetos SfaFactura en un objeto DataSet
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns> 
    ...
    ''' <summary>
    ''' Trae de la base de datos un conjunto de objetos SfaFactura en un objeto DataReader
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns>
...
#Region " Insert "
	...
	''' <summary>
    ''' 	Inserta en la base de datos una nueva instancia de la clase SfaFactura.
    ''' </summary>	
    ''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Insert.</param>
    ''' <remarks></remarks>
	Public Sub Insert(Optional ByRef pTransac As TransactionManager = Nothing)
		Dim sCommand As String = "insert into SfaFactura("
		sCommand &= "Fecha,"
		...
		sCommand &= " select "
		sCommand &= "@SfaFacturaID = SfaFacturaID from SfaFactura where "		
        ...
        End If
		arParams(13) = New SqlParameter("@SccCuentaID", SqlDbType.VarChar)		
		If IsDBNull(m_SccCuentaID) Then
            arParams(13).Value = DBNull.Value
        Else
            arParams(13).Value = m_SccCuentaID
        End If
...
#Region " Update "
	...
	''' <summary>
    ''' 	Actualiza el objeto SfaFactura en la base de datos.
    ''' </summary>	
    ''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    ''' <remarks></remarks>
	Public Sub Update(Optional ByRef pTransac As TransactionManager = Nothing)        		
		Dim sCommand As String = "update SfaFactura set "		
...
#Region " Delete "
	...
	''' <summary>
    ''' Borra un objeto SfaFactura de la base de datos.
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    ''' </summary>    
    Public Sub Delete(optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from SfaFactura where " & " SfaFacturaID = " & m_SfaFacturaID
			If pTransac Is Nothing Then
    ...
    Public Shared Sub Delete(Byval p_SfaFacturaID as Integer, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from SfaFactura where " & " SfaFacturaID = " & p_SfaFacturaID
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
    ...
    ''' <summary>
    ''' Borra objetos SfaFactura de la base de datos en base al par?metro WHERE
    ''' </summary>
    ''' <param name="pWhere">Condici?n usada para borrar los objetos de la base de datos.</param>
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    Public Shared Sub DeleteByFilter(Byval pWhere as String, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from SfaFactura where " & pWhere
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
