...
Partial Public Class SfaFacturasDetalle		

#Region " Variables Miembro " 
	Protected m_SfaFacturaDetalleID As Integer 
	Protected m_objSfaFacturaID As Integer 
...
#Region " Propiedades "
	Public Property SfaFacturaDetalleID() As Integer
        Get
            Return (m_SfaFacturaDetalleID)
        End Get
		Set(ByVal Value As Integer)					
			m_SfaFacturaDetalleID = Value
		End Set
    End Property
	
	Public Property objSfaFacturaID() As Integer
        Get
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 30 Then
					Throw New ArgumentOutOfRangeException("UsuarioCreacion", Value.ToString(), "Valor inv?lido para SfaFacturasDetalle.UsuarioCreacion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (30).")
				End If
			End If
	...
	Public Property UsuarioModificacion() As String
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 30 Then
					Throw New ArgumentOutOfRangeException("UsuarioModificacion", Value.ToString(), "Valor inv?lido para SfaFacturasDetalle.UsuarioModificacion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (30).")
				End If
			End If
...
#Region " Batch Update " 
	''' <summary>
    ''' Salva un DataSet del tipo de la tabla SfaFacturasDetalle en la base de datos.
    ''' </summary>
    ''' <param name="DS">Dataset a salvar. El DS debe necesariamente ser un select a la tabla SfaFacturasDetalle </param>    
	...
	public Shared Sub BatchUpdate( DS as Dataset, optional Byval pTransac as TransactionManager = Nothing )
		...
		Try
			...
			cmdDelete.Parameters.Add("@SfaFacturaDetalleID", SqlDbType.Int, 4, "SfaFacturaDetalleID" )
			cmdDelete.CommandText = "DELETE FROM  SfaFacturasDetalle WHERE SfaFacturaDetalleID= @SfaFacturaDetalleID" 	
			...
			cmdInsert.CommandText = "INSERT INTO SfaFacturasDetalle ( objSfaFacturaID, objSivProductoID, Cantidad, Precio, Subtotal, Descuento, Impuesto, Total, CostoDolares, objEntradaBodegaID, ExistenciaAnterior, UsuarioCreacion, FechaCreacion, UsuarioModificacion, FechaModificacion) VALUES ( @objSfaFacturaID, @objSivProductoID, @Cantidad, @Precio, @Subtotal, @Descuento, @Impuesto, @Total, @CostoDolares, @objEntradaBodegaID, @ExistenciaAnterior, @UsuarioCreacion, @FechaCreacion, @UsuarioModificacion, @FechaModificacion)"
			...
			cmdUpdate.Parameters.Add("@wSfaFacturaDetalleID", SqlDbType.Int, 4, "SfaFacturaDetalleID")
			cmdUpdate.CommandText = "UPDATE SfaFacturasDetalle SET objSfaFacturaID=@objSfaFacturaID, objSivProductoID=@objSivProductoID, Cantidad=@Cantidad, Precio=@Precio, Subtotal=@Subtotal, Descuento=@Descuento, Impuesto=@Impuesto, Total=@Total, CostoDolares=@CostoDolares, objEntradaBodegaID=@objEntradaBodegaID, ExistenciaAnterior=@ExistenciaAnterior, UsuarioCreacion=@UsuarioCreacion, FechaCreacion=@FechaCreacion, UsuarioModificacion=@UsuarioModificacion, FechaModificacion=@FechaModificacion WHERE SfaFacturaDetalleID= @wSfaFacturaDetalleID"
			...
			sqlHelper.UpdateDataset (cmdInsert, cmdDelete, cmdUpdate, DS, "SfaFacturasDetalle")
...
#Region " Retrieve "
	''' <summary>
    ''' Devuelve de la base de datos un objeto SfaFacturasDetalle
    ''' </summary>    
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Retrieve.</param>    	
    Public Function Retrieve(Byval p_SfaFacturaDetalleID as Integer, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from SfaFacturasDetalle where " & " SfaFacturaDetalleID = " & p_SfaFacturaDetalleID		
        ...
        Try
			...
			If dr.Read() Then		
				m_SfaFacturaDetalleID = dr("SfaFacturaDetalleID")
	...
	''' <summary>
    ''' Devuelve de la base de datos un objeto SfaFacturasDetalle usando un filter.
    ''' </summary>
    ''' <param name="pFilter">Filtro a aplicar en el select que carga la clase</param>    
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Retrieve.</param>    
    Public Function RetrieveByFilter(ByVal pFilter as String, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from SfaFacturasDetalle where " & pFilter				
        Dim dr As SqlDataReader = Nothing
	...
	''' <summary>
    ''' Trae de la base de datos un conjunto de objetos SfaFacturasDetalle en un objeto DataTable
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns> 
    Public Shared Function RetrieveDT(Optional ByVal sFilter As String = "", Optional ByVal OrderBy As String = "", Optional Byval pFields as String = "*", optional Byval pTransac as TransactionManager = Nothing) As DataTable
        Dim sSQL As String = "select " & pFields &" from SfaFacturasDetalle"
		...
		Try
			...
			End If
			ds.Tables(0).TableName = "SfaFacturasDetalle"
			Return (ds.Tables(0))
		Catch ex As Exception
			Throw        
        End Try		
	...
	''' <summary>
    ''' Trae de la base de datos un conjunto de objetos SfaFacturasDetalle en un objeto DataSet
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns> 
    ...
    ''' <summary>
    ''' Trae de la base de datos un conjunto de objetos SfaFacturasDetalle en un objeto DataReader
    ''' </summary>
    ''' <param name="sFilter">Condici?n que filtra los objetos a traer (WHERE)</param>
    ''' <param name="OrderBy">Campos por los que se ordenar? la salida</param>
    ''' <returns></returns>
...
#Region " Insert "
	...
	''' <summary>
    ''' 	Inserta en la base de datos una nueva instancia de la clase SfaFacturasDetalle.
    ''' </summary>	
    ''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el m?todo Insert.</param>
    ''' <remarks></remarks>
	Public Sub Insert(Optional ByRef pTransac As TransactionManager = Nothing)
		Dim sCommand As String = "insert into SfaFacturasDetalle("
		...
		sCommand &= " select "
		sCommand &= "@SfaFacturaDetalleID = SfaFacturaDetalleID from SfaFacturasDetalle where "		
		sCommand &= "SfaFacturaDetalleID = SCOPE_IDENTITY()"
		
		
		Dim arParams(15) As SqlParameter
		arParams(0) = New SqlParameter("@SfaFacturaDetalleID", SqlDbType.Int)		
		arParams(0).Direction = ParameterDirection.Output
		arParams(1) = New SqlParameter("@objSfaFacturaID", SqlDbType.Int)		
		If IsDBNull(m_objSfaFacturaID) Then
            arParams(1).Value = DBNull.Value
        Else
            arParams(1).Value = m_objSfaFacturaID
        End If
        ...
        End If
		arParams(7) = New SqlParameter("@Impuesto", SqlDbType.Decimal)		
		If IsDBNull(m_Impuesto) Then
            arParams(7).Value = DBNull.Value
        Else
            arParams(7).Value = m_Impuesto
        End If
		arParams(8) = New SqlParameter("@Total", SqlDbType.Decimal)		
		If IsDBNull(m_Total) Then
            arParams(8).Value = DBNull.Value
        Else
            arParams(8).Value = m_Total
        End If
		arParams(9) = New SqlParameter("@CostoDolares", SqlDbType.Decimal)		
		If IsDBNull(m_CostoDolares) Then
            arParams(9).Value = DBNull.Value
        Else
            arParams(9).Value = m_CostoDolares
        End If
		arParams(10) = New SqlParameter("@objEntradaBodegaID", SqlDbType.Int)		
		If IsDBNull(m_objEntradaBodegaID) Then
            arParams(10).Value = DBNull.Value
        Else
            arParams(10).Value = m_objEntradaBodegaID
        End If
		arParams(11) = New SqlParameter("@ExistenciaAnterior", SqlDbType.Int)		
		If IsDBNull(m_ExistenciaAnterior) Then
            arParams(11).Value = DBNull.Value
        Else
            arParams(11).Value = m_ExistenciaAnterior
        End If
		...
		Try
			...
			m_SfaFacturaDetalleID = arParams(0).Value
        Catch ex As Exception
            Throw        
        End Try
    End Sub	
...
#Region " Update "
	...
	''' <summary>
    ''' 	Actualiza el objeto SfaFacturasDetalle en la base de datos.
    ''' </summary>	
    ''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    ''' <remarks></remarks>
	Public Sub Update(Optional ByRef pTransac As TransactionManager = Nothing)        		
		Dim sCommand As String = "update SfaFacturasDetalle set "		
		...
		sCommand &= " where "	
		sCommand &= "SfaFacturaDetalleID = @SfaFacturaDetalleID"					
		
		Dim arParams(15) As SqlParameter
		arParams(0) = New SqlParameter("@SfaFacturaDetalleID", SqlDbType.Int)		
		If IsDBNull(m_SfaFacturaDetalleID) Then
            arParams(0).Value = DBNull.Value
        Else
            arParams(0).Value = m_SfaFacturaDetalleID
        End If
		arParams(1) = New SqlParameter("@objSfaFacturaID", SqlDbType.Int)		
		If IsDBNull(m_objSfaFacturaID) Then
            arParams(1).Value = DBNull.Value
...
#Region " Delete "
	...
	''' <summary>
    ''' Borra un objeto SfaFacturasDetalle de la base de datos.
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    ''' </summary>    
    Public Sub Delete(optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from SfaFacturasDetalle where " & " SfaFacturaDetalleID = " & m_SfaFacturaDetalleID
			If pTransac Is Nothing Then
    ...
    ''' Borra un objeto SfaFacturasDetalle de la base de datos
    ''' </summary>    
    Public Shared Sub Delete(Byval p_SfaFacturaDetalleID as Integer, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from SfaFacturasDetalle where " & " SfaFacturaDetalleID = " & p_SfaFacturaDetalleID
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
    ...
    ''' <summary>
    ''' Borra objetos SfaFacturasDetalle de la base de datos en base al par?metro WHERE
    ''' </summary>
    ''' <param name="pWhere">Condici?n usada para borrar los objetos de la base de datos.</param>
	''' <param name="pTransac">Opcional. La transacci?n en cuyo ?mbito ser? ejecutado el Update del objeto.</param>
    Public Shared Sub DeleteByFilter(Byval pWhere as String, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from SfaFacturasDetalle where " & pWhere
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
