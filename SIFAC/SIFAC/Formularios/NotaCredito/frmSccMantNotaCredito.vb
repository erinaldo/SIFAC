...
''-- Fecha de Elaboración    : 19 de Marzo de 2009.
''----------------------------------------------------------------------
...
#Region "Eventos del Formulario"
    ...
    '-- Fecha de Creación   :   03 de Diciembre del 2007
    ...
    Private Sub frmSccMantNotaCredito_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            '-- Ocultar las Columnas del Grid
            Me.grdNotaCredito.Splits(0).DisplayColumns("SccNotaCreditoID").Visible = False
            Me.grdNotaCredito.Splits(0).DisplayColumns("objEstadoID").Visible = False
            Me.grdNotaCredito.Splits(0).DisplayColumns("objConceptoID").Visible = False
...
#Region "Grid"
    Private Sub CargarGridNotaCredito()
        Try
            ...
            If dtNC.Rows.Count > 0 Then
                ...
                Me.grdNotaCredito.SetDataBinding(dtNC, "", True)
                Me.grdNotaCredito.Caption = "Notas de Crédito (" & dtNC.Rows.Count & ")"
                dtNC.PrimaryKey = New DataColumn() {dtNC.Columns("SccNotaCreditoID")}
                Me.dtNC.DefaultView.Sort = "SccNotaCreditoID"
            End If

            If Me.grdNotaCredito.RowCount > 0 Then
                Me.NCID = Me.grdNotaCredito.Columns("SccNotaCreditoID").CellValue(Me.grdNotaCredito.RowBookmark(Me.grdNotaCredito.Row))
                Me.EstadoNC = Me.grdNotaCredito.Columns("objEstadoID").CellValue(Me.grdNotaCredito.RowBookmark(Me.grdNotaCredito.Row))
                Me.ConceptoNC = Me.grdNotaCredito.Columns("objConceptoID").CellValue(Me.grdNotaCredito.RowBookmark(Me.grdNotaCredito.Row))
    ...
    Private Sub grdNotaCredito_AfterFilter(ByVal sender As Object, ByVal e As C1.Win.C1TrueDBGrid.FilterEventArgs) Handles grdNotaCredito.AfterFilter
        Try
            Me.grdNotaCredito.Caption = "Notas de Crédito (" & dtNC.DefaultView.Count & ")"
    ...
    Private Sub grdNotaCredito_RowColChange(ByVal sender As Object, ByVal e As C1.Win.C1TrueDBGrid.RowColChangeEventArgs) Handles grdNotaCredito.RowColChange
        Try
            If Me.grdNotaCredito.RowCount > 0 Then
                Me.NCID = Me.grdNotaCredito.Columns("SccNotaCreditoID").CellValue(Me.grdNotaCredito.RowBookmark(Me.grdNotaCredito.Row))
                Me.EstadoNC = Me.grdNotaCredito.Columns("objEstadoID").CellValue(Me.grdNotaCredito.RowBookmark(Me.grdNotaCredito.Row))
...
#Region "Eventos del Toolbar"
    ...
    '-----------------------------------------------------------------------------------
    Private Sub tsbModificarNC_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbModificarNC.Click, grdNotaCredito.DoubleClick
        Try
    ...
    Private Sub ModificarNotaCredito()
        ...
        Try
            ...
            objfrm = New frmSccEditNotaCredito
            objfrm.TypeGui = 1
            objfrm.IDNotaCredito = Me.grdNotaCredito.Columns("SccNotaCreditoID").Value
            '-- Reservar Registro
    ...
    Private Sub tsbAnularNC_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbAnularNC.Click
        ...
        Try
            If dtNC.DefaultView.Count > 0 And blnAnularNC And (EstadoNC = EstadoRegistrada Or EstadoNC = EstadoAutorizada) Then
                ...
                If Me.grdNotaCredito.Columns("objEstadoID").Value <> IDEstadoVig Then
        ...
        Try
            ...
            Select Case MsgBox("¿Seguro que desea anular la nota de crédito?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, clsProyecto.SiglasSistema)
                Case MsgBoxResult.Yes
                    ...
                    T.CommitTran()
                    CargarGridNotaCredito()
                    For Each dr As DataRow In dtNC.Rows
                        If dr("SccNotaCreditoID") = IDTemp Then
                            Me.grdNotaCredito.Row = dtNC.Rows.IndexOf(dr)
                            Exit For
                        End If
                    Next
                    MsgBox(My.Resources.MsgActualizado, MsgBoxStyle.Information + MsgBoxStyle.OkOnly, clsProyecto.SiglasSistema)
