...
''-- Fecha de Elaboración    : 04 de Diciembre del 2007.
''----------------------------------------------------------------------
''--    Formulario de Parámetros de Búsqueda de Notas de Crédito
''----------------------------------------------------------------------
Imports DAL
Imports SCCUM.BO
...
Public Class frmSccParamBuscarNotaCredito
...
#Region "Declaracion de Variables Locales y Objetos"
    ...
    Private m_TipoNCID As DataTable
    Private m_TipoCerosRelleno As Integer
#End Region

#Region "Propiedades"
    Public Property TipodtNCID() As DataTable
        Get
            Return m_TipoNCID
        End Get
...
#Region "Eventos del Formulario"
    ...
    '-----------------------------------------------------------------------------------
    Private Sub frmSccParamBuscarNotaCredito_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
...
#Region "Combos"
    ...
    '-- Nombre del Autor    :   Michelle Valezka Reyes Tijerino
    '-- Fecha de Creación   :   04 de Diciembre del 2007
    '-- Descripcion         :   Cargar el Combo Estado con los valores del Catalogo EstadoNC.
    '----------------------------------------------------------------------------------------------------------
    Private Sub CargarEstado()
        Try
            strCampos = " Descripcion, StbValorCatalogoID "
            strFiltro = " Nombre='EstadoNC' "
    ...
    Private Sub cmdGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdGuardar.Click
        ...
        Try
            If ValidarDatos(0) Then
                ...
                '-- Verificar el Parametro la Busqueda. Si es Libre o por Numero de Nota de Credito
                If Me.chkNumero.Checked Then
                    ...
                    End If
                Else
                    '-- Parametro Rango de Fechas en que se realizó la Nota de Credito
                    ...
                    '-- Parametro Estado de la Nota de Credito
                    ...
                    '-- Parametro Cliente a quien pertenece la Nota de Credito
                    If Me.txtCliente.Text.Trim.Length > 0 Then
                        strCliente = " AND objClienteID= " & ClienteID
                    End If

                    '-- Parametro de Sucursal del Cliente a la que se realizó la Nota de Credito
                ...
                strCampos = " * "
                strOrden = " Fecha "
                strSQL = clsConsultas.ObtenerNotaCredito(strCampos, TipoFiltro, strOrden)

                '-- Obtener el Datatable con el Resultado de la Consulta
                If Me.chkNumero.Checked Then
                    Me.TipodtNCID = SqlHelper.ExecuteQueryDT(strSQL)
                Else
                    '-- Verificar que arreglo de parametros de fechas asignar a la consulta
                    If blnParametro Then
                        Me.TipodtNCID = SqlHelper.ExecuteQueryDT(strSQL, ParmFechas)
                    Else
                        Me.TipodtNCID = SqlHelper.ExecuteQueryDT(strSQL, aParmFechas)
                    End If
                End If

                If Me.TipodtNCID.Rows.Count = 0 Then
