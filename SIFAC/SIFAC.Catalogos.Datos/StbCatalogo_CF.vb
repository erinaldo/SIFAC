...
Partial Public Class StbCatalogo		

#Region " Variables Miembro " 
	Protected m_StbCatalogoID As Integer	
...
#Region " Propiedades "
	Public Property StbCatalogoID() As Integer
        Get
            Return (m_StbCatalogoID)
        End Get
		Set(ByVal Value As Integer)					
			m_StbCatalogoID = Value
		End Set
    End Property
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 50 Then
					Throw New ArgumentOutOfRangeException("Nombre", Value.ToString(), "Valor inv?lido para StbCatalogo.Nombre. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (50).")
				End If
			End If
			m_Nombre = Value
		End Set
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 250 Then
					Throw New ArgumentOutOfRangeException("Descripcion", Value.ToString(), "Valor inv?lido para StbCatalogo.Descripcion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (250).")
				End If
			End If
			m_Descripcion = Value
		End Set
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 30 Then
					Throw New ArgumentOutOfRangeException("UsuarioCreacion", Value.ToString(), "Valor inv?lido para StbCatalogo.UsuarioCreacion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (30).")
				End If
			End If
	...
	Public Property UsuarioModificacion() As String
		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 30 Then
					Throw New ArgumentOutOfRangeException("UsuarioModificacion", Value.ToString(), "Valor inv?lido para StbCatalogo.UsuarioModificacion. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (30).")
				End If
			End If
...
#Region " Batch Update " 
	...
	public Shared Sub BatchUpdate( DS as Dataset, optional Byval pTransac as TransactionManager = Nothing )
		...
		Try
			...
			cmdDelete.Parameters.Add("@StbCatalogoID", SqlDbType.Int, 4, "StbCatalogoID" )
			cmdDelete.CommandText = "DELETE FROM  StbCatalogo WHERE StbCatalogoID= @StbCatalogoID" 	
			...
			cmdInsert.CommandText = "INSERT INTO StbCatalogo ( Nombre, Descripcion, UsuarioCreacion, FechaCreacion, UsuarioModificacion, FechaModificacion, Activo) VALUES ( @Nombre, @Descripcion, @UsuarioCreacion, @FechaCreacion, @UsuarioModificacion, @FechaModificacion, @Activo)"
			...
			cmdUpdate.Parameters.Add("@wStbCatalogoID", SqlDbType.Int, 4, "StbCatalogoID")
			cmdUpdate.CommandText = "UPDATE StbCatalogo SET Nombre=@Nombre, Descripcion=@Descripcion, UsuarioCreacion=@UsuarioCreacion, FechaCreacion=@FechaCreacion, UsuarioModificacion=@UsuarioModificacion, FechaModificacion=@FechaModificacion, Activo=@Activo WHERE StbCatalogoID= @wStbCatalogoID"
			...
			sqlHelper.UpdateDataset (cmdInsert, cmdDelete, cmdUpdate, DS, "StbCatalogo")
...
#Region " Retrieve "
    ...
    Public Function Retrieve(Byval p_StbCatalogoID as Integer, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from StbCatalogo where " & " StbCatalogoID = " & p_StbCatalogoID		
        ...
        Try
			...
			If dr.Read() Then
				m_StbCatalogoID = dr("StbCatalogoID")
    ...
    Public Function RetrieveByFilter(ByVal pFilter as String, optional Byval pTransac as TransactionManager = Nothing) As Boolean
        Dim sSQL As String = "select * from StbCatalogo where " & pFilter				
        Dim dr As SqlDataReader = Nothing
    ...
    Public Shared Function RetrieveDT(Optional ByVal sFilter As String = "", Optional ByVal OrderBy As String = "", Optional Byval pFields as String = "*", optional Byval pTransac as TransactionManager = Nothing) As DataTable
        Dim sSQL As String = "select " & pFields &" from StbCatalogo"
		...
		Try
			...
			End If
			ds.Tables(0).TableName = "StbCatalogo"
			Return (ds.Tables(0))
		Catch ex As Exception
			Throw        
        End Try		
...
#Region " Insert "
	...
	Public Sub Insert(Optional ByRef pTransac As TransactionManager = Nothing)
		Dim sCommand As String = "insert into StbCatalogo("
		sCommand &= "Nombre,"
		...
		sCommand &= " select "
		sCommand &= "@StbCatalogoID = StbCatalogoID from StbCatalogo where "		
		sCommand &= "StbCatalogoID = SCOPE_IDENTITY()"
		
		
		Dim arParams(7) As SqlParameter
		arParams(0) = New SqlParameter("@StbCatalogoID", SqlDbType.Int)		
		...
		Try
			...
			m_StbCatalogoID = arParams(0).Value
        Catch ex As Exception
            Throw        
        End Try
    End Sub	
...
#Region " Update "
	...
	Public Sub Update(Optional ByRef pTransac As TransactionManager = Nothing)        		
		Dim sCommand As String = "update StbCatalogo set "		
		...
		sCommand &= " where "	
		sCommand &= "StbCatalogoID = @StbCatalogoID"					
		
		Dim arParams(7) As SqlParameter
		arParams(0) = New SqlParameter("@StbCatalogoID", SqlDbType.Int)		
		If IsDBNull(m_StbCatalogoID) Then
            arParams(0).Value = DBNull.Value
        Else
            arParams(0).Value = m_StbCatalogoID
        End If
		arParams(1) = New SqlParameter("@Nombre", SqlDbType.VarChar)		
		If IsDBNull(m_Nombre) Then
            arParams(1).Value = DBNull.Value
...
#Region " Delete "
    ...
    Public Sub Delete(optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from StbCatalogo where " & " StbCatalogoID = " & m_StbCatalogoID
			If pTransac Is Nothing Then
    ...
    ''' Borra un objeto StbCatalogo de la base de datos
    ''' </summary>    
    Public Shared Sub Delete(Byval p_StbCatalogoID as Integer, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from StbCatalogo where " & " StbCatalogoID = " & p_StbCatalogoID
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
    ...
    Public Shared Sub DeleteByFilter(Byval pWhere as String, optional Byval pTransac as TransactionManager = Nothing)
		Try
			Dim sSQL As String = "delete from StbCatalogo where " & pWhere
			If pTransac Is Nothing Then
				SqlHelper.ExecuteNonQuery(CommandType.Text, sSQL)
			Else
				SqlHelper.ExecuteNonQuery(pTransac.Transaction, CommandType.Text, sSQL)
